...
public class CleanupJobRunner implements JobRunner {
    ...
    private int cleanupOldUsageRecords(int retentionDays) {
        ...
        int cleaned = 0;
        try {
            com.mycompany.confluence.aigeneration.ao.AoUsageRecord[] old = ao.find(
                    com.mycompany.confluence.aigeneration.ao.AoUsageRecord.class,
                    Query.select().where("RECORD_DATE < ?", cutoff));

            for (com.mycompany.confluence.aigeneration.ao.AoUsageRecord r : old) {
                ao.delete(r);
                cleaned++;
            }
